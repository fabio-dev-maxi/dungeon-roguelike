  ...
  private async simulateSingleRun(cls: ClassKey) {
    ...
    for (let depth = 1; depth <= 50; depth++) {
      ...
      if (s.phase === 'combat' && s.monster) {
        ...
        while (s.player!.hp > 0 && s.monster && s.monster.hp > 0 && rounds < 100) {
            ...
            if (s.player!.hp < s.player!.maxHp * 0.35 && s.player!.inventory.some(i => i.type === 'potion')) {
                await this.combatService.playerUsePotion();
                       ...
                       (s.player!.cls === 'fighter' || s.player!.cls === 'wizard' || (s.player!.cls === 'cleric' && s.player!.hp < s.player!.maxHp * 0.6))) {
        ...
        if (s.player!.hp <= 0) {
            isDead = true;
            return { died: true, depth: currentDepth, bossId: isBoss ? mId : null, bossEncounters };
        }
        ...
        await this.processPendingLevelUps();
          ...
          if (choiceOpt) {
             if(s.pendingChoice.canFail) {
                 ...
                 if (s.player!.hp <= 0) {
                     return { died: true, depth: currentDepth, bossId: null, bossEncounters };
                 }
             }
          }
      }
    ...
    return { died: false, depth: 50, bossId: null, bossEncounters };
  }
