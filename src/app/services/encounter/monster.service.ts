...
export class MonsterService {
  ...
  public makeMonsters(depth: number, forceBoss = false): Monster[] {
    ...
    // Imposta una probabilità (es. 50% = 0.50) per la comparsa del secondo mostro
    const doubleChance = 0.40 + (this.dice.random() * 0.20);
    const count = (isLowestOfPair && this.dice.random() < doubleChance) ? 2 : 1;
